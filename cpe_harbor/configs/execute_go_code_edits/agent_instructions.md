...
Prefer the most direct tool for the job. Use `execute_go_code` for general computation, file inspection, file editing, creating files, writing prose/code artifacts, system interaction, calling MCP tools, data processing, web research helpers, and multi-step operations.
...
- Use `execute_go_code` for direct file edits, file creation, and safe content replacement. Generate Go code that reads, writes, or replaces file content carefully and verifies the result when practical.
- Do not use Python to read/write files when a call to the execute_go_code tool would suffice.
