...
func TestRefreshIsSerializedAndPreservesOtherCredentials(t *testing.T) {
...
func TestRefreshFailuresDoNotLeakOrRewriteCredentials(t *testing.T) {
...
func TestRefreshWithoutRotatedTokenPreservesRefresh(t *testing.T) {
	path := filepath.Join(t.TempDir(), "auth.json")
	writeAuthFixture(t, path, time.Now().Add(-time.Hour))
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
...
func TestCredentialLockCancellationAndRelease(t *testing.T) {
...
func TestNewStartsWithoutCredentialsAndRejectsInvalidCredentialsOnUse(t *testing.T) {
	path := filepath.Join(t.TempDir(), "auth.json")
	if New(path) == nil {
	...
	if _, err := s.credential(t.Context(), false); err == nil || strings.Contains(err.Error(), "secret-in-error") {
...
func TestCredentialReloadObservesLoginChanges(t *testing.T) {
	path := filepath.Join(t.TempDir(), "auth.json")
	before := writeAuthFixture(t, path, time.Now().Add(time.Hour))
	s := &credentialStore{path: path}
	...
	after := strings.Replace(string(before), "fixture-access", "updated-by-pi", 1)
	if err := os.WriteFile(path, []byte(after), 0600); err != nil {
		t.Fatal(err)
	}
	second, err := s.credential(t.Context(), true)
	if err != nil || second.access != "updated-by-pi" {
		t.Fatalf("credential change was not reloaded: %v", err)
	}
}

func TestAccountClaimAndRefreshRotation(t *testing.T) {
	path := filepath.Join(t.TempDir(), "auth.json")
	writeAuthFixture(t, path, time.Now().Add(-time.Hour))
	...
	for _, invalid := range []string{"bad", "a.%%%.b", "a." + base64.RawURLEncoding.EncodeToString([]byte(`{}`)) + ".b"} {
		if accountFromToken(invalid) != "" {
			t.Fatal("invalid claim accepted")
		}
	}
}

func TestCanceledRefreshReleasesLockWithoutChangingFile(t *testing.T) {
	path := filepath.Join(t.TempDir(), "auth.json")
	before := writeAuthFixture(t, path, time.Now().Add(-time.Hour))
...
func TestTokenRefreshDoesNotFollowRedirects(t *testing.T) {
	path := filepath.Join(t.TempDir(), "auth.json")
	writeAuthFixture(t, path, time.Now().Add(-time.Hour))
