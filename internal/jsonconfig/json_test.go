...
func TestDecode(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "field casing", input: `{"Name":"x"}`, wantErr: true},
		{name: "alias duplicate", input: `{"name":"x","Name":"y"}`, wantErr: true},
		{name: "nested casing", input: `{"child":{"Name":"x"}}`, wantErr: true},
		{name: "map casing", input: `{"labels":{"A":1,"a":2}}`, want: configuration{Labels: map[string]int{"A": 1, "a": 2}}},
