...
func TestInitGeneratorFromModel(t *testing.T) {
	t.Run("wraps all models with network and provider retry and only responses with phase retry", func(t *testing.T) {
	...
	t.Run("anthropic vertex requires vertex config", func(t *testing.T) {
...
func TestVertexScopes(t *testing.T) {
	original := findDefaultGoogleCredentials
	t.Cleanup(func() { findDefaultGoogleCredentials = original })
	...
	tests := []struct {
		...
		{
			name: "cloud platform default",
			want: []string{googleCloudPlatformScope},
		},
		{
			name:       "custom scopes",
			configured: []string{"https://example.test/scope-a", "https://example.test/scope-b"},
			want:       []string{"https://example.test/scope-a", "https://example.test/scope-b"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var got []string
			findDefaultGoogleCredentials = func(_ context.Context, scopes ...string) (*google.Credentials, error) {
				got = append([]string(nil), scopes...)
				return &google.Credentials{
					TokenSource: oauth2.StaticTokenSource(&oauth2.Token{AccessToken: "token"}),
				}, nil
			}
			_, err := anthropicVertexRequestOptions(t.Context(), &config.VertexConfig{
				ProjectID: "project",
				Region:    "global",
			...
			if !slices.Equal(got, tt.want) {
				t.Fatalf("credential scopes = %#v, want %#v", got, tt.want)
			}
		})
	}
}

func TestAnthropicVertexRequestOptions(t *testing.T) {
	t.Run("use google auth and vertex request shape", func(t *testing.T) {
	...
	t.Run("do not retry resource exhausted", func(t *testing.T) {
		const modelID = "claude-sonnet-4-6"
		requestCount := 0
		var bodies []string
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	t.Run("apply patch request", func(t *testing.T) {
		const modelID = "claude-sonnet-4-6"
		requestCount := 0
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			requestCount++
...
func TestThinkingSummaryApplicationForResponses(t *testing.T) {
	tests := []struct {
		name           string
		opts           *gai.GenOpts
		wantExtraArgs  bool
