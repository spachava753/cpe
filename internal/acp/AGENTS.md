...
- Before invoking a tool callback, attach the storage ID of its persisted assistant message through `xctx.WithExecutionMessageID`. Code Mode's `acp.get_session()` uses that cutoff because the ACP session head is not advanced until the prompt turn finishes; do not replace it with a session-head lookup or tool-call-ID scan.
