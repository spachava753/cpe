...
func TestModule(t *testing.T) {
	t.Run("reads typed current and persisted sessions", func(t *testing.T) {
		t.Parallel()
...
print(latest.messages[-2].role, latest.messages[-2].tool_result_error, latest.messages[-2].blocks[0].content)`)
	...
	t.Run("returns complete compaction history", func(t *testing.T) {
		t.Parallel()
...
for message in session.messages:
	...
	t.Run("returns empty persisted session", func(t *testing.T) {
		t.Parallel()
